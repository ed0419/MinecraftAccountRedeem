...
with open('mc25.acc', newline='', encoding = 'utf-8') as csvfile:
    ...
    for rows in rows_stu:
        cursor = db.cursor()
        cursor.execute("INSERT INTO MC_NEW (email, password) VALUES (%(uid)s,%(pass)s)",{'uid':str(rows[0])+"A"*10, 'pass':rows[1]})
        db.commit()
        print(rows)